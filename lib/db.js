...
const vfa    = require('very-fast-args');
...
class LimitDB {
  constructor(params) {
    ...
    if (params.inMemory) {
      //This is a workaround this issue:
      // https://github.com/Level/memdown/issues/66
      this._db.put = (function(originalPut) {
        return function(key) {
          const args = vfa.apply(null, arguments);
          args[0] = Buffer.from(key);
          console.log('get', args[0]);
          originalPut.apply(this, args);
        };
      })(this._db.put);
      this._db.get = (function(originalGet) {
        return function(key) {
          const args = vfa.apply(null, arguments);
          args[0] = Buffer.from(key);
          console.log('get', args[0]);
