...
class LimitDBRedis extends EventEmitter {
  ...
  scanStream(params) {
    ...
    return mergeStream(nodes.map((node) => {
      return node.scanStream({
        match: `${params.type}:${params.prefix}*`,
        count: 10,
      });
    }));
  }
  ...
  buildStatusResponse(bucket, prefixLength) {
    return (key, cb) => {
      // In order to be compatible with cluster, pipeline can't be used
      this.redis.hmget(key, 'last_drip', 'content', (err, hash) => {
        if (err) {
          return cb(err);
        }
  ...
  status(params, callback) {
    ...
    // TODO: Accept a limit
    const bucket = this.buckets[params.type];
    const prefixLength = `${params.type}:`.length;
    const transform = this.buildStatusResponse(bucket, prefixLength);

    var matches = [];

    const stream = this.scanStream(params);
    ...
    stream.on('end', () => {
      async.map(matches, transform, (err, res) => {
        return callback(err, { items: res });
      });
    });
