...
class LimitDBRedis extends EventEmitter {
  ...
  validateParams(params, attr) {
    if (typeof params !== 'object') {
      return new ValidationError('params are required');
    }

    if (typeof params.type !== 'string') {
      return new ValidationError('type is required');
    }

    if (typeof this.buckets[params.type] === 'undefined') {
      return new ValidationError(`undefined bucket type ${params.type}`);
    }

    if (typeof params[attr] !== 'string') {
      return new ValidationError(`${attr} is required`);
    }
  }
